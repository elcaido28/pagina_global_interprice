<?php include('cabecera.php'); include('menu.php'); include('conexion.php'); $id_fecha=$_REQUEST['idfech'];?>
<div class="contenido">
  <div class="content_tabla_listas">
    <div class="titulo_testimonio">
      <h3>Lista de Paquetes de Guías Aereas - <?php echo $id_fecha ?></h3> <input type="button" id="save_value" name="" value="Generar Facturas">
    </div>
    <div class="comentar">
      <div class="content_table">
        ...
        <table class="tabla">
          ...
          $cons4="SELECT * FROM todas_facturas where feha_subida_arch='$id_fecha' ";
            $ejec4=mysqli_query($con,$cons4);
            while ($row4=mysqli_fetch_array($ejec4)) {
          ?>
            <td><input type="checkbox" class="checkBoxF" name="" value="F-<?php echo $row4['id_todas_facturas']; ?>"></td>
            <td><input type="checkbox" class="checkBoxG" name="" value="G-<?php echo $row4['id_todas_facturas']; ?>"></td>
            <td><input type="checkbox" class="checkBoxD" name="" value="D-<?php echo $row4['id_todas_facturas']; ?>"></td>
            <td><input type="checkbox" class="checkBoxR" name="" value="R-<?php echo $row4['id_todas_facturas']; ?>"></td>
          ...
          $('#save_value').click(function() {
            ...
            .done(function(respuesta){
              ...
              for (var i = 0; i < num_datos; i++) {
                ...
                if (dt_id[0]=="F") {
                    alert(dt_id[1]);
                    var url_g="reporte_factura2.php?idf="+dt_id[1];
                    document.location.target = "_blank";
                    document.location.href=url_g;
                }
                if (dt_id[0]=="G") {
                    alert(dt_id[1]);
                    var url_g="reporte_guia2.php?idf="+dt_id[1];
                    document.location.target = "_blank";
                    document.location.href=url_g;
                }
                if (dt_id[0]=="D") {
                    alert(dt_id[1]);
                    var url_g="reporte_declaracion2.php?idf="+dt_id[1];
                    document.location.target = "_blank";
                    document.location.href=url_g;
                }
                if (dt_id[0]=="R") {
                    alert(dt_id[1]);
                    var url_g="reporte_recibo_en2.php?idf="+dt_id[1];
                    document.location.target = "_blank";
                    document.location.href=url_g;
                }
